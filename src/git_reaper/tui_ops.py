...
OPERATIONS: list[Operation] = [
    Operation(
        "limbs", "limbs - hierarchical file listing", "reaping", False, _limbs,
        (_format_opt("md", "json"),),
    ),
    ...
    Operation(
        "census", "census - file-type census", "packing", False, _census,
        (_format_opt(*_ALL_FORMATS),),
    ),
    Operation(
        "unfinished", "unfinished - TODO/FIXME/HACK/XXX", "packing", False, _unfinished,
        (ToggleOpt("age", "age (how long it has haunted)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "bones", "bones - structure without the flesh", "packing", False, _bones,
        (_format_opt("md", "json"),),
    ),
    Operation(
        "chronicle", "chronicle - commit history", "necromancy", True, _chronicle,
        (ToggleOpt("changelog", "changelog (group by tag)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "souls", "souls - contributors", "necromancy", True, _souls,
        (ToggleOpt("heatmap", "heatmap", default=True), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "haunt", "haunt - churn hotspots", "necromancy", True, _haunt,
        (NumberOpt("limit", "limit (top N)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "graveyard", "graveyard - files that lived and died", "necromancy", True, _graveyard,
        (_format_opt(*_ALL_FORMATS),),
    ),
    Operation(
        "rot", "rot - staleness report", "necromancy", True, _rot,
        (NumberOpt("limit", "limit (top N)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "ghosts", "ghosts - branch hygiene", "necromancy", True, _ghosts,
        (TextOpt("than", "than (e.g. 90d)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "tombstone", "tombstone - the stats card", "necromancy", True, _tombstone,
        (_format_opt("md", "json"),),
    ),
    Operation(
        "doppelgangers", "doppelgangers - duplicate files", "forensics", False, _doppelgangers,
        (TextOpt("min_size", "min size (e.g. 4KB)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "bloat", "bloat - the heaviest bodies", "forensics", False, _bloat,
        (NumberOpt("limit", "limit (top N)", default=20), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "exhume", "exhume - secrets in the history", "dark arts", True, _exhume,
        (ToggleOpt("no_entropy", "no entropy (signatures only)"), _format_opt(*_ALL_FORMATS)),
    ),
    Operation(
        "omens", "omens - composite risk prophecy", "dark arts", True, _omens,
        (
    ...
    Operation(
        "plague", "plague - dependency advisories", "dark arts", False, _plague,
